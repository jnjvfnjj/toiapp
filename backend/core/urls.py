...
assets_path = settings.BASE_DIR.parent / 'frontend' / 'build' / 'assets'
...
if settings.DEBUG:
    # Serve Vite dev server assets (если используется dev режим)
        src_path = settings.BASE_DIR.parent / 'frontend' / 'src'
        if src_path.exists():
            urlpatterns.append(re_path(r'^src/(?P<path>.*)$', serve, {
                'document_root': src_path
            }))
    
        node_modules_path = settings.BASE_DIR.parent / 'frontend' / 'node_modules'
